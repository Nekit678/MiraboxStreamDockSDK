...
class Action(Generic[SettingsT, DependenciesT]):
    ...
    The connection invokes callbacks sequentially for each action context;
    callbacks for different contexts may overlap. Action fields and their
    mutable JSON views are therefore context-local callback state, not
    concurrently mutable application state. A background service should use
    its injected :class:`StreamDockSender` directly instead of mutating an
    action instance.
