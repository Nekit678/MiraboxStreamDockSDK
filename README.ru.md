...
На уровне `INFO` записываются направление протокола, событие и контекст. `DEBUG`
добавляет маршрутизирующие метаданные. Payload остаётся скрытым, если только
явно не настроено `include_payload=True`. Handlers, установленные приложением
вручную, остаются его ответственностью.
