...
- [Контракт конкурентности](#контракт-конкурентности)
...
Runtime явно распределяет владение между потоками:
...
Повторный вызов сначала дренирует очередь и заменяет handler, ранее созданный
`configure_logging()`, поэтому уровень и назначение можно менять без
дублирования сообщений. Для сброса ожидающих записей и возврата в исходный
бесшумный режим:
...
`include_payload=True`. SDK передаёт записи одному управляемому logging-потоку,
